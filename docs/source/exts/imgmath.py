...
def compile_math(latex: str, builder: Builder) -> str:
    ...
    command = builder.config.imgmath_latex.replace("CWD", tempdir).split()
...
def convert_dvi_to_png(dvipath: str, builder: Builder) -> Tuple[str, int]:
    ...
    command = builder.config.imgmath_dvipng.replace("CWD", tempdir).split()
    command.extend(['-o', filename, '-T', 'tight', '-z9'])
