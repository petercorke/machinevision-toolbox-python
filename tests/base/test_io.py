...
from machinevisiontoolbox.base.imageio import convert, idisp, iwrite
...
# Stripe fixtures are in tests/data/ and were generated by examples/stripe.py.
# They are committed to the repository so that these tests are independent of
# iwrite / convert correctness.
#
# Layout (1000 × 400 px, uint8):
#   stripe 0 (u 0–199):   black   RGB = [0, 0, 0]
#   stripe 1 (u 200–399): red     RGB = [255, 0, 0]
#   stripe 2 (u 400–599): green   RGB = [0, 255, 0]
#   stripe 3 (u 600–799): blue    RGB = [0, 0, 255]
#   stripe 4 (u 800–999): white   RGB = [255, 255, 255]
#
# Sampling coordinates: v = 200 (centre row), u = 100 / 300 / 500 / 700 / 900

_STRIPE_DATA = Path(__file__).parent.parent / "data"
_STRIPES_RGB = _STRIPE_DATA / "stripes.png"
_STRIPES_RGBA = _STRIPE_DATA / "stripes_a.png"

# v=200 (centre row), stripe-centre u values and expected RGB pixels
_CENTRES = [
    (100, [0, 0, 0]),  # black
    (300, [255, 0, 0]),  # red
    (500, [0, 255, 0]),  # green
...
class TestBaseStripeIO(unittest.TestCase):
    """Tests for iread / idisp using pre-committed stripe fixture images."""
    ...
    def test_iread_rgb(self):
        """iread default: 3-channel RGB array."""
        im, path = iread(str(_STRIPES_RGB))
        self.assertEqual(im.shape, (400, 1000, 3))
        self.assertEqual(im.dtype, np.uint8)
        for u, expected_rgb in _CENTRES:
            nt.assert_array_equal(
                im[_V, u], expected_rgb, err_msg=f"RGB mismatch at u={u}"
            )

    def test_iread_bgr(self):
        """iread rgb=False: 3-channel BGR array – R and B channels swapped."""
        im, _ = iread(str(_STRIPES_RGB), rgb=False)
        self.assertEqual(im.shape, (400, 1000, 3))
        # Convert expected RGB → BGR for comparison
        for u, rgb in _CENTRES:
            bgr = [rgb[2], rgb[1], rgb[0]]
            nt.assert_array_equal(im[_V, u], bgr, err_msg=f"BGR mismatch at u={u}")

    def test_iread_rgba_file_default(self):
        """iread RGBA file with default alpha=False: alpha stripped → RGB."""
        im, _ = iread(str(_STRIPES_RGBA))
        self.assertEqual(im.shape, (400, 1000, 3))
        for u, expected_rgb in _CENTRES:
            nt.assert_array_equal(
                im[_V, u], expected_rgb, err_msg=f"RGBA→RGB mismatch at u={u}"
            )

    def test_iread_rgba(self):
        """iread RGBA file with alpha=True: 4-channel RGBA array."""
        im, _ = iread(str(_STRIPES_RGBA), alpha=True)
        self.assertEqual(im.shape, (400, 1000, 4))
        for u, rgb in _CENTRES:
            nt.assert_array_equal(
                im[_V, u], rgb + [255], err_msg=f"RGBA mismatch at u={u}"
            )

    def test_iread_bgra(self):
        """iread RGBA file with rgb=False, alpha=True: BGRA array."""
        im, _ = iread(str(_STRIPES_RGBA), rgb=False, alpha=True)
        self.assertEqual(im.shape, (400, 1000, 4))
        for u, rgb in _CENTRES:
            bgra = [rgb[2], rgb[1], rgb[0], 255]
            nt.assert_array_equal(im[_V, u], bgra, err_msg=f"BGRA mismatch at u={u}")

    # ------------------------------------------------------------------
    # idisp – check displayed pixel values via AxesImage.get_array()
    # idisp returns the AxesImage handle h; np.asarray(h.get_array()) gives
    # the uint8 array exactly as passed to imshow (already in RGB/RGBA order).
    # ------------------------------------------------------------------

    def _disp_pixel(self, h, v, u):
        """Return displayed pixel at (v, u) as a plain numpy array."""
        return np.asarray(h.get_array())[v, u]

    def test_idisp_rgb(self):
        """idisp with RGB array: displayed pixels match RGB values."""
        im, _ = iread(str(_STRIPES_RGB))
        h = idisp(im, colororder="RGB", block=None)
        for u, expected_rgb in _CENTRES:
            nt.assert_array_equal(
                self._disp_pixel(h, _V, u),
                expected_rgb,
                err_msg=f"idisp RGB mismatch at u={u}",
            )

    def test_idisp_bgr(self):
        """idisp with BGR array + colororder='BGR': displayed pixels are RGB."""
        im, _ = iread(str(_STRIPES_RGB), rgb=False)
        h = idisp(im, colororder="BGR", block=None)
        for u, expected_rgb in _CENTRES:
            nt.assert_array_equal(
                self._disp_pixel(h, _V, u),
                expected_rgb,
                err_msg=f"idisp BGR→RGB mismatch at u={u}",
            )

    def test_idisp_rgba(self):
        """idisp with RGBA array: displayed pixels include alpha channel."""
        im, _ = iread(str(_STRIPES_RGBA), alpha=True)
        h = idisp(im, colororder="RGBA", block=None)
        for u, rgb in _CENTRES:
            nt.assert_array_equal(
                self._disp_pixel(h, _V, u),
                rgb + [255],
                err_msg=f"idisp RGBA mismatch at u={u}",
            )

    def test_idisp_bgra(self):
        """idisp with BGRA array + colororder='BGRA': displayed pixels are RGBA."""
        im, _ = iread(str(_STRIPES_RGBA), rgb=False, alpha=True)
        h = idisp(im, colororder="BGRA", block=None)
        for u, rgb in _CENTRES:
            nt.assert_array_equal(
                self._disp_pixel(h, _V, u),
                rgb + [255],
                err_msg=f"idisp BGRA→RGBA mismatch at u={u}",
            )
