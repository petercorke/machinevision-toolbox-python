...
def loadspectrum(lam, filename='bb2.dat'):
    ...
    # This is hacky if you change the filename
    tab = np.asarray(tab.reshape(-1, 4), dtype=float)
    Q = []
    for i in range(1, 4):
        # This will give us the interpolation function
        f = interpolate.interp1d(tab[:, 0], tab[:, i], kind='cubic')
        tmp = f(lam)
        Q.append(tmp)
...
def sharpen_matrix(x):
    theta = x[0]
    phi = x[1]
    c1 = np.array([math.sin(theta)*math.cos(phi),
                   math.sin(theta)*math.sin(phi), math.cos(theta)])
    c1 = c1.transpose()

    theta = x[2]
    phi = x[3]
    c2 = np.array([math.sin(theta)*math.cos(phi),
                   math.sin(theta)*math.sin(phi), math.cos(theta)])
    c2 = c2.transpose()

    theta = x[4]
    phi = x[5]
    c3 = np.array([math.sin(theta)*math.cos(phi),
                   math.sin(theta)*math.sin(phi), math.cos(theta)])
    c3 = c3.transpose()
...
def costfunc(x, spectrum):
    ...
    overlap = np.sum((sharp[:, 0]*sharp[:, 1])) + np.sum((sharp[:, 0]
                                                          * sharp[:, 2])) + np.sum((sharp[:, 1]*sharp[:, 2]))
...
def log_chrom_coord(image):
    ...
    r_r = np.divide(im[:, 0], denom, out=np.zeros(
        im[:, 0].shape, dtype=float), where=denom != 0)
    r_b = np.divide(im[:, 2], denom, out=np.zeros(
        im[:, 0].shape, dtype=float), where=denom != 0)
...
gs_im = invariant(gamma_corrected_img, theta)
